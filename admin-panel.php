...
// Fetch session data and ensure values are set
...
// Check if the session data is set, else redirect to login page or show an error message
if (empty($pid) || empty($username)) {
...
if(isset($_POST['app-submit'])) {
  ...
  // Convert appointment date and time to timestamps for comparison
  $apptime1 = strtotime($apptime);
  $appdate1 = strtotime($appdate);

  // Check if the selected appointment date and time are valid
  if(date("Y-m-d", $appdate1) >= $cur_date) {
    if((date("Y-m-d", $appdate1) == $cur_date && date("H:i:s", $apptime1) > $cur_time) || date("Y-m-d", $appdate1) > $cur_date) {
      ...
      if ($result->num_rows == 0) {
        // Insert new appointment using prepared statement
      ...
      echo "<script>alert('Select a future time or date!');</script>";
    }
  } else {
    echo "<script>alert('Select a future time or date!');</script>";
  }
...
.btn-primary{
  ...
  <div class="col-md-8" style="margin-top: 3%;">
    ...
    <div class="container-fluid">
        ...
        <a href="test.php" class="btn btn-primary">Open Chat</a>
    </div>
</div>
  ...
  <table class="table table-hover">
    <thead>
      <tr>
        ...
        <th scope="col">Payment Status</th> <!-- Payment Status column -->
        <th scope="col">Pay Now</th> <!-- Separate Pay Now column -->
        <th scope="col">Cancel Appointment</th> <!-- Separate Cancel column -->
      </tr>
    </thead>
    <tbody>
      <?php 
        ...
        // Query to fetch appointment details along with payment status
        $query = "SELECT ID, doctor, docFees, appdate, apptime, userStatus, doctorStatus, payment_status 
                  FROM appointmenttb 
      ...
      ?>
          <tr>
            ...
            <!-- Pay Now Button in separate column -->
            <td>
              <?php 
                if (($row['userStatus'] == 1) && ($row['doctorStatus'] == 1) && $row['payment_status'] == 'Pending') { ?>
                <form action="pay.php" method="POST">
                  <input type="hidden" name="appointment_id" value="<?php echo $row['ID']; ?>">
                  <input type="hidden" name="doctor" value="<?php echo $row['doctor']; ?>">
                  <input type="hidden" name="docFees" value="<?php echo $row['docFees']; ?>">
                  <button type="submit" class="btn btn-primary">Pay Now</button>
                </form>
              <?php } else {
                echo "-"; // Placeholder if payment isn't pending
              } ?>
            </td>

            <!-- Cancel Button in separate column -->
            <td>
              <?php 
                if (($row['userStatus'] == 1) && ($row['doctorStatus'] == 1)) { ?>
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <!-- <a href="admin-panel.php?ID=" 
                        ...
                        <input type = "submit" onclick="alert('Bill Paid Successfully');" name ="generate_bill" class = "btn btn-success" value="Bill"/>
                        </a>
                        </td>
                        </form>
